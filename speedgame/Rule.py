...
class Resource(metaclass=StaticClassMeta):
    """Lớp **Resource** : lưu trữ thông tin các tài nguyên chuẩn của thư viện."""

    __doc__ = 'Lớp Resource : lưu trữ thông tin các tài nguyên chuẩn của thư viện.'

    icon_dir = 'speedgame/InterfaceIcons'
    font_dir = 'speedgame/InterfaceFonts'
    font_name = 'JetBrains'
    font_save = {}

    # Lấy tất cả tên asset có trong folder InterfaceIcons, InterfaceFonts
    InterfaceIcons = {splitext(path)[0]: path for path in os.listdir(icon_dir)}
    InterfaceFonts = {splitext(path)[0]: path for path in os.listdir(font_dir)}
...
def load_asset(dir_name: str):
    def decorator(load_method):
        def wrapper(self, asset_name: str):
            assert asset_name in getattr(Resource, basename(dir_name)),\
                f'Tài nguyên tên {asset_name} không tồn tại trong thư mục {dir_name} !'

            path = f'{dir_name}/{getattr(Resource, basename(dir_name))[asset_name]}'
