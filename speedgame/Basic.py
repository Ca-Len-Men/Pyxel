...
from speedgame.Rule import SingletonMeta, StaticClassMeta, Resource, load_asset, basename, splitext
...
class Vector2:
    ...
    @classmethod
    def copy_from(cls, point: Union[Tuple[int, int], Tuple[float, float]]):
        """ Phương thức **copy_from** : tạo một *Vector2* từ một *tuple*.
...
class FontStore(metaclass=SingletonMeta):
    ...
    def __getitem__(self, asset_name):
        key = splitext(basename(asset_name))[0]
        if key not in Resource.font_save:
            Resource.font_save[key] = Font(asset_name)
        return Resource.font_save[key]
