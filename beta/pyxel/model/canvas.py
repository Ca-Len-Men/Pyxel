...
from pyxel.model.components.sprite import CSprite
...
class CCanvas(CSprite):
    def __init__(self, source, canvas):
        ...
        self._CONSTANT_SPRITE = CSprite(source.copy())
    ...
    def update(self):
        ...
        self.copy_from_source(self._CONSTANT_SPRITE)
    ...
    def blit_surf(self, surface, position_type):
        if isinstance(position_type, str):
            surface_rect = Rect(Vector(*surface.get_size()))
...
class Canvas:
    ...
    def update_mouse_position(self):
        if self._canvas:
            self._mouse_position = self._canvas.mpos - self.rect.topleft
        else:
